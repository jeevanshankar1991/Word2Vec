...
class VocabBuilder(vocab_hash_size : Int = 20e6.toInt, sampling_table_size : Int = 1e8.toInt, load_factor : Double = 0.7) {
           ...
           def addWordToVocab(key : String) : Unit = {
                 ...
                 if (vocab_size > vocab_hash_size * load_factor) {
                      reduce_vocab(min_reduce) // keeps only those words in the vocab whose count is greater than min_reduce
                      min_reduce += 1
                 }
          ...
          // helper functions 
           private def reduce_vocab(min_reduce : Int) : Unit =  {
                 var a = 0
                 var b = 0
                 // filter the words whose cn <= min_reduce 
           ...
           private def rehash() : Unit = {
              (0 until vocab_hash_size).par.foreach(i => vocab_hash(i) = -1)
              for (a <- 0 until vocab_size) {
...
class vocab_word(count : Int = 0, w : String)  {
       var cn = count
       var word = w
